...
const string kValue = "some really long value that takes a lot of space";

class InsertVerifyITest : public KuduTest {
  ...
  void SetUp() override {
    ...
    opts.extra_tserver_flags.push_back("--vmodule=log_block_manager=4");
...
TEST_F(InsertVerifyITest, TestInsertAndVerify) {
  ...
  for (int i = 1; i < num_columns; i++) {
    b.AddColumn(strings::Substitute("value-$0", i))->Type(KuduColumnSchema::INT64)->NotNull();
  }
  ...
  auto deadline = MonoTime::Now() + MonoDelta::FromSeconds(100);
  ...
  long count = 0;
  while (MonoTime::Now() < deadline) {
    // Upsert a block of [500, 1500) random rows.
