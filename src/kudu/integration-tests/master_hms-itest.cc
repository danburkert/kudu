...
TEST_F(MasterHmsUpgradeTest, TestConflictingNormalizedNames) {
  ...
  // immediately try to elect itself leader, and CHECK fail upon seing the
  // conflicting table names. However, in TSAN or otherwise slow situations the
  // master may be able to register all tablet servers before it elects itself
  // leader, so the Restart() call may suceed. In these situations we
  // additionally call an API which requires leadership to add a secondary
  // check.
