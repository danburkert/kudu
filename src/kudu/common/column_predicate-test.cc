...
void test_merge(const ColumnSchema& column,
                const ColumnPredicate& a,
...
template <typename T>
void test_merge_combinations(const ColumnSchema& column,
                             vector<T> values) {
  // Range + Range
  ...
  //   [---)
  test_merge(column,
             ColumnPredicate::Range(column, &values[0], &values[3]),
...
TEST(ColumnPredicateTest, TestMerge) {
  test_merge_combinations(ColumnSchema("c", INT8),
                          vector<int8_t> { 0, 1, 2, 3, 4, 5, 6 });

  test_merge_combinations(ColumnSchema("c", INT32),
                          vector<int32_t> { -100, -10, -1, 0, 1, 10, 100 });

  test_merge_combinations(ColumnSchema("c", STRING),
                          vector<Slice> { "a", "b", "c", "d", "e", "f", "g" });

  test_merge_combinations(ColumnSchema("c", BINARY),
...
// Test that the range constructor handles equality and disjoint ranges.
TEST(ColumnPredicateTest, TestRangeConstructor) {
  {
...
TEST(ColumnPredicateTest, TestInclusiveRange) {
