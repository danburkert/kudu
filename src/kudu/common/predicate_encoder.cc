...
void RangePredicateEncoder::LiftPrimaryKeyBounds(const ScanSpec& spec,
  ...
  if (lower_bound_key && exclusive_upper_bound_key) {
    ...
    num_lower_bound_predicates = std::min<int>(prefix_equalities + 1, key_schema_->num_key_columns());
    num_upper_bound_predicates = std::min<int>(prefix_equalities + 1, key_schema_->num_key_columns());
