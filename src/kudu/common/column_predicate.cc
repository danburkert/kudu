...
ColumnPredicate ColumnPredicate::Range(ColumnSchema column,
  ...
  if (lower != nullptr && upper != nullptr) {
    // Check if the range is disjoint or an equality.
    if (column.type_info()->Compare(lower, upper) >= 0) {
      // If the range bounds are disjoint then no results can be returned.
      return ColumnPredicate::None(column);
    } else if (column.type_info()->AreConsecutive(lower, upper)) {
      // If the values are consecutive, then it is an equality bound.
      return ColumnPredicate::Equality(move(column), lower);
    }
  }

  return ColumnPredicate(PredicateType::Range, move(column), lower, upper);
}
...
void ColumnPredicate::MergeIntoRange(const ColumnPredicate& other) {
  ...
  switch (other.predicate_type()) {
    ...
    case PredicateType::Range: {
      ...
      // Check if the resulting range is disjoint or an equality.
  ...
  switch (predicate_type()) {
    ...
    case PredicateType::Range: {
      if (lower_ == nullptr) {
        ...
        return strings::Substitute("(`$0` BETWEEN $1 AND $2)",
                                   column_.name(),
