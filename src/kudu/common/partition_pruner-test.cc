...
void CheckPrunedPartitions(const Schema& schema,
                            ...
                            const ScanSpec& spec,
                            size_t expected_pruned_tablets) {
  ...
  ASSERT_EQ(expected_pruned_tablets, pruned_partitions);
}
...
TEST(TestPartitionPruner, TestPrimaryKeyRangePruning) {
  ...
  auto Check = [&] (optional<tuple<int8_t, int8_t, int8_t>> lower,
    ...
    CheckPrunedPartitions(schema, partition_schema, partitions, spec, expected_pruned_tablets);
  };

  // No bounds
  Check(boost::none, boost::none, 0);
...
TEST(TestPartitionPruner, TestPartialPrimaryKeyRangePruning) {
  ...
  // PK < (-1, min, _)
  Check(boost::none, make_tuple<int8_t, string>(-1, ""), 2);

  // PK < (10, "r", _)
  Check(boost::none, make_tuple<int8_t, string>(10, "r"), 1);

  // PK < (100, min)
  Check(boost::none, make_tuple<int8_t, string>(100, ""), 0);

  // PK >= (-10, "m")
  Check(make_tuple<int8_t, string>(-10, "m"), boost::none, 0);

  // PK >= (0, "")
  Check(make_tuple<int8_t, string>(0, ""), boost::none, 0);

  // PK >= (0, "m")
  Check(make_tuple<int8_t, string>(0, "m"), boost::none, 1);

  // PK >= (100, "")
  Check(make_tuple<int8_t, string>(100, ""), boost::none, 2);

  // PK >= (-10, 0)
  // PK  < (100, 0)
  Check(make_tuple<int8_t, string>(-10, ""),
        make_tuple<int8_t, string>(100, ""), 0);

  // PK >= (0, "m")
  // PK  < (10, "r")
  Check(make_tuple<int8_t, string>(0, "m"),
        make_tuple<int8_t, string>(10, "r"), 2);
...
TEST(TestPartitionPruner, TestRangePruning) {
  ...
  auto Check = [&] (const vector<ColumnPredicate>& predicates, size_t expected_pruned_tablets) {
    ScanSpec spec;
  ...
  // No Bounds
  Check(vector<ColumnPredicate> {}, 0);

  // c < -10
  Check(vector<ColumnPredicate> {
        ColumnPredicate::Range(schema.column(2), nullptr, &neg_ten)
  }, 2);

  // c = -10
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Equality(schema.column(2), &neg_ten)
  }, 2);
  ...
  // c < 100
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), nullptr, &hundred)
  }, 0);

  // c >= -10
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(0), &neg_ten, nullptr)
  }, 0);

  // c >= 0
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), &zero, nullptr)
  }, 0);

  // c >= 5
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), &five, nullptr)
  }, 1);
  ...
  // c >= 100
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), &hundred, nullptr)
  }, 2);

  // -10 <= c < 0
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), &neg_ten, &zero)
  }, 2);
  ...
  // 5 <= c < 100
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), &five, &hundred)
  }, 1);

  // b = ""
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Equality(schema.column(1), &empty)
  }, 0);

  // b >= "z"
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(1), &z, nullptr)
  }, 0);

  // b < "a"
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(1), nullptr, &a)
  }, 0);

  // "m" <= b < "z"
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(1), &m, &z)
  }, 0);
  ...
  // b < "m"
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(2), nullptr, &zero),
  ...
  // b = "m\0"
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Equality(schema.column(2), &zero),
...
TEST(TestPartitionPruner, TestHashPruning) {
  ...
  // a = 0;
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Equality(schema.column(0), &zero),
  }, 2);

  // a >= 0;
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(0), &zero, nullptr),
  }, 0);

  // a >= 0;
  // a < 1;
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(0), &zero, &one),
  }, 2);

  // a >= 0;
  // a < 2;
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Range(schema.column(0), &zero, &two),
  }, 0);
...
TEST(TestPartitionPruner, TestPruning) {
  ...
  // timestamp >= 9;
  Check(vector<ColumnPredicate> {
      ColumnPredicate::Equality(schema.column(0), &a),
