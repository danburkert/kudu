...
class ClientTest : public KuduTest {
 ...
 protected:
  ...
  unique_ptr<KuduInsert> BuildTestInsert(KuduTable* table, int index) {
  ...
  unique_ptr<KuduInsertIgnore> BuildTestInsertIgnore(KuduTable* table, int index) {
  ...
  void PopulateDefaultRow(KuduPartialRow* row, int index) {
...
static void DoTestInsertIgnoreVerifyRows(const shared_ptr<KuduTable>& tbl, int numRows) {
    vector<string> rows;
...
TEST_F(ClientTest, TestInsertIgnore) {
...

  {
    ...
    NO_FATALS(DoTestInsertIgnoreVerifyRows(client_table_, 1));
  }
...

  {
    ...
    NO_FATALS(DoTestInsertIgnoreVerifyRows(client_table_, 2));
  }
}
