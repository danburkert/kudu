...
Status MetaCache::ProcessLookupResponse(const LookupRpc& rpc,
  ...
  if (rpc.resp().tablet_locations().empty()) {
    // The table has no tablets. This could also indicate that the partition key
    // range in the GetTableLocationsRequestPB falls before all tablets, but
    // since we never set an upper bound, that is impossible.
