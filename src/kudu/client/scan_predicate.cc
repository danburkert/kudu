...
Status ComparisonPredicateData::AddToScanSpec(ScanSpec* spec, Arena* arena) {
  ...
  switch (op_) {
    case KuduPredicate::LESS_EQUAL: {
      auto pred = ColumnPredicate::InclusiveRange(move(col_), nullptr, val_void, arena);
      if (pred) {
    ...
    case KuduPredicate::GREATER_EQUAL: {
      spec->AddPredicate(ColumnPredicate::Range(move(col_), val_void, nullptr));
      break;
    };
    case KuduPredicate::EQUAL: {
      spec->AddPredicate(ColumnPredicate::Equality(move(col_), val_void));
      break;
    };
    default:
