...
Status HaClient<Service>::Start(std::vector<HostPort> addresses, ClientOptions options) {
  if (threadpool_) {
    return Status::IllegalState("Thrift service client is already started");
  }
...
Status HaClient<Service>::Execute(std::function<Status(Service*)> task) {
  ...
  RETURN_NOT_OK(threadpool_->SubmitFunc([=] {
    ...
    for (int attempt = 0; attempt <= options_.retry_count; attempt++) {
      ...
      VLOG(1) << "Call to " << Service::kServiceName << " failed: " << task_status.ToString();
    ...
    LOG(WARNING) << "Call to " << Service::kServiceName << " failed after "
                 << options_.retry_count << " retries: " << first_failure.ToString();
...
Status HaClient<Service>::Reconnect() {
  ...
  for (int i = 0; i < addresses_.size(); i++) {
    ...
    if (s.ok()) {
      VLOG(1) << "Connected to " << Service::kServiceName << " " << address.ToString();
      return Status::OK();
    }
