...
void SaslClientTransport::ReadFrame() {
  ...
  read_buf_.resize(kFrameHeaderSize);
  ...
  */
  ...
  if (needs_wrap_) {
    ...
    Status s = rpc::SaslDecode(sasl_conn_.get(), read_buf_, &plaintext);
    if (!s.ok()) {
...
void SaslClientTransport::Negotiate() {
  ...
  LOG(INFO) << "SASL GSSAPI negotiation complete; needs wrap: " << (needs_wrap_ ? "true" : "false")
