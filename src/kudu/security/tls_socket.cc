...
TlsSocket::TlsSocket(int fd, SSL* ssl)
    : Socket(fd),
      ssl_(ssl) {
}
...
Status TlsSocket::Write(const uint8_t *buf, int32_t amt, int32_t *nwritten) {
  ...
  if (bytes_written <= 0) {
    auto error_code = SSL_get_error(ssl_, bytes_written);
...
Status TlsSocket::Recv(uint8_t *buf, int32_t amt, int32_t *nread) {
  ...
  if (bytes_read <= 0) {
    ...
    auto error_code = SSL_get_error(ssl_, bytes_read);
...
Status TlsSocket::Close() {
  ...
  // response, since the underlying socket will not be resused.
  int32_t ret = SSL_shutdown(ssl_);
  Status ssl_shutdown;
  if (ret >= 0) {
    ssl_shutdown = Status::OK();
  } else {
    auto error_code = SSL_get_error(ssl_, ret);
  ...
  // TODO(dan): consider putting this in ReactorThread::Shutdown
  ERR_remove_state(0);
