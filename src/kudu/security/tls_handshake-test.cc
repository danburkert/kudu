...
struct Case {
  Case(CertType client_cert_type,
       TlsVerificationMode client_verification_mode,
       CertType server_cert_type,
       TlsVerificationMode server_verification_mode,
       Status expected_status_)
      : client_cert(client_cert_type),
...
TEST_F(TestTlsHandshake, TestHandshakeSequence) {
  ...
  InitTlsContextCert(ca_key, ca_cert, &client_tls_, CertType::SIGNED);
  InitTlsContextCert(ca_key, ca_cert, &server_tls_, CertType::SIGNED);
...
TEST_F(TestTlsHandshake, TestTlsContextCertTransition) {
  ...
  ASSERT_STR_MATCHES(server_tls_.AdoptSignedCert(bogus_cert).ToString(),
                     "signed certificate public key does not match the CSR public key");
...
TEST_P(TestTlsHandshake, TestHandshake) {
  ...
  InitTlsContextCert(ca_key, ca_cert, &client_tls_, test_case.client_cert);
  InitTlsContextCert(ca_key, ca_cert, &server_tls_, test_case.server_cert);
