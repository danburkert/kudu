...
TEST_F(TestTlsHandshake, TestHandshake) {
  ...
  ASSERT_OK(rpc::CreateSSLServerCert(cert_path));
  ASSERT_OK(rpc::CreateSSLPrivateKey(key_path));
  ...
  tls_context.InitiateHandshake(true, &server);
  tls_context.InitiateHandshake(false, &client);
