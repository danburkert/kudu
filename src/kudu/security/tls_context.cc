...
Status TlsContext::Init() {
  ...
  if (!ctx_) {
    return Status::RuntimeError("Could not create TLS context", GetOpenSSLErrors());
  }
...
Status TlsContext::LoadCertificate(const std::string& certificate_path) {
  ERR_clear_error();
  errno = 0;
...
Status TlsContext::LoadPrivateKey(const std::string& key_path) {
  ERR_clear_error();
  errno = 0;
...
Status TlsContext::LoadCertificateAuthority(const std::string& certificate_path) {
  ERR_clear_error();
  errno = 0;
...
Status TlsContext::InitiateHandshake(bool is_server, TlsHandshake* handshake) const {
  CHECK(ctx_);
