...
static int ServerNegotiationGetoptCb(ServerNegotiation* sasl_server,
                                     const char* plugin_name,
                                     const char* option,
                                     const char** result,
                                     unsigned* len) {
  return sasl_server->GetOptionCb(plugin_name, option, result, len);
}

static int ServerNegotiationPlainAuthCb(sasl_conn_t *conn,
                                        ServerNegotiation* sasl_server,
                                        const char* user,
                                        const char* pass,
                                        unsigned passlen,
                                        struct propctx *propctx) {
  return sasl_server->PlainAuthCb(conn, user, pass, passlen, propctx);
}
...
Status ServerNegotiation::RecvNegotiatePB(NegotiatePB* msg, faststring* recv_buf) {
  ...
  Status s = helper_.CheckSaslCallId(header.call_id());
  if (!s.ok()) {
...
Status ServerNegotiation::SendError(ErrorStatusPB::RpcErrorCodePB code, const Status& err) {
  ...
  TRACE("Sent RPC error: $0", ErrorStatusPB::RpcErrorCodePB_Name(code));
  return Status::OK();
}
...
Status ServerNegotiation::HandleSaslInitiate(const NegotiatePB& request) {
  if (PREDICT_FALSE(request.step() != NegotiatePB::SASL_INITIATE)) {
    Status s =  Status::InvalidArgument("expected SASL_INITIATE step",
                                        NegotiatePB::NegotiateStep_Name(request.step()));
    RETURN_NOT_OK(SendError(ErrorStatusPB::FATAL_UNAUTHORIZED, s));
    return s;
  }
  ...
  if (request.auths_size() != 1) {
    Status s = Status::NotAuthorized(strings::Substitute(
          "SASL_INITIATE request must include exactly one SaslAuth section, found: $0",
          request.auths_size()));
    RETURN_NOT_OK(SendError(ErrorStatusPB::FATAL_UNAUTHORIZED, s));
    return s;
  }
...
Status ServerNegotiation::HandleSaslResponse(const NegotiatePB& request) {
  if (PREDICT_FALSE(request.step() != NegotiatePB::SASL_RESPONSE)) {
    Status s =  Status::InvalidArgument("expected SASL_RESPONSE step",
                                        NegotiatePB::NegotiateStep_Name(request.step()));
    RETURN_NOT_OK(SendError(ErrorStatusPB::FATAL_UNAUTHORIZED, s));
    return s;
  }
...
Status ServerNegotiation::SendSaslChallenge(const char* challenge, unsigned clen) {
  ...
  TRACE("Sending SASL_CHALLENGE response to client");
  RETURN_NOT_OK(SendNegotiatePB(response));
...
Status ServerNegotiation::SendSaslSuccess(const char* token, unsigned tlen) {
  ...
  TRACE("Sending SASL_SUCCESS response to client");
  RETURN_NOT_OK(SendNegotiatePB(response));
  return Status::OK();
}
