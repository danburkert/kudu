...
static int ClientNegotiationGetoptCb(ClientNegotiation* sasl_client,
                                     const char* plugin_name,
                                     const char* option,
                                     const char** result,
                                     unsigned* len) {
  return sasl_client->GetOptionCb(plugin_name, option, result, len);
}

static int ClientNegotiationSimpleCb(ClientNegotiation* sasl_client,
                                     int id,
                                     const char **result,
                                     unsigned *len) {
  return sasl_client->SimpleCb(id, result, len);
}

static int ClientNegotiationSecretCb(sasl_conn_t* conn,
                                     ClientNegotiation* sasl_client,
                                     int id,
                                     sasl_secret_t** psecret) {
  return sasl_client->SecretCb(conn, id, psecret);
}
...
Status ClientNegotiation::Negotiate() {
  ...
  if (tls_context_ && ContainsKey(server_features_, TLS)) {
    RETURN_NOT_OK(tls_context_->InitiateHandshake(false, &tls_handshake_));

    // To initiate the TLS handshake, we pretend as if the server sent us a
    // TLS_HANDSHAKE message with an empty token.
    NegotiatePB initial;
  ...
  for (bool cont = true; cont; ) {
    ...
    switch (response.step()) {
      ...
      case NegotiatePB::SASL_SUCCESS:
        ...
        break;
      default:
        return Status::InvalidArgument("expected SASL_CHALLENGE or SASL_SUCCESS step",
...
Status ClientNegotiation::RecvNegotiatePB(NegotiatePB* msg, faststring* buffer) {
  ...
  RETURN_NOT_OK(helper_.CheckSaslCallId(header.call_id()));

  if (header.is_error()) {
    return ParseError(param_buf);
  }

  return helper_.ParseNegotiatePB(param_buf, msg);
}
...
Status ClientNegotiation::InitSaslClient() {
  ...
  RETURN_NOT_OK(WrapSaslCall(nullptr /* no conn */, [&]() {
...
Status ClientNegotiation::SendNegotiate() {
  ...
  TRACE("Sending NEGOTIATE request to server.");
...
Status ClientNegotiation::HandleNegotiate(const NegotiatePB& response) {
  if (PREDICT_FALSE(response.step() != NegotiatePB::NEGOTIATE)) {
    return Status::InvalidArgument("expected NEGOTIATE step",
                                   NegotiatePB::NegotiateStep_Name(response.step()));
  }
...
Status ClientNegotiation::SendTlsHandshake(const string& tls_token) {
...
Status ClientNegotiation::HandleTlsHandshake(const NegotiatePB& response) {
  if (PREDICT_FALSE(response.step() != NegotiatePB::TLS_HANDSHAKE)) {
    return Status::InvalidArgument("expected TLS_HANDSHAKE step",
                                   NegotiatePB::NegotiateStep_Name(response.step()));
  }
  ...
  if (PREDICT_FALSE(!response.has_tls_handshake())) {
    return Status::InvalidArgument(
        "No TLS handshake message in TLS_HANDSHAKE response from server");
  }
...
Status ClientNegotiation::SendSaslInitiate() {
  ...
  TRACE("Sending SASL_INITIATE request to server.");
  return SendNegotiatePB(msg);
}
...
Status ClientNegotiation::SendSaslResponse(const char* resp_msg, unsigned resp_msg_len) {
  ...
  TRACE("Sending SASL_RESPONSE request to server.");
