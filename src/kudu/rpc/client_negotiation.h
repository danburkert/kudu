...
class ClientNegotiation {
 public:
  ...
  // Allow TLS to be used on the connection. 'tls_context' is borrowed, and must
  // outlive this ClientNegotiation.
  ...
  std::unique_ptr<Socket> take_socket() { return std::move(socket_); }
 ...
 private:
  ...
  Status SendTlsHandshake(const std::string& tls_token) WARN_UNUSED_RESULT;
