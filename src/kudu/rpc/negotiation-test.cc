...
TEST_F(TestNegotiation, TestBasicInit) {
  ServerNegotiation server(nullptr);
  server.EnablePlain();
  ASSERT_OK(server.Init());
  ClientNegotiation client(nullptr);
...
TEST_F(TestNegotiation, TestPreflight) {
  ...
  ASSERT_OK(ServerNegotiation::PreflightCheckGSSAPI(kSaslAppName));

  // Try with an inaccessible keytab.
  CHECK_ERR(chmod(kt_path.c_str(), 0000));
  s = ServerNegotiation::PreflightCheckGSSAPI(kSaslAppName);
  ASSERT_FALSE(s.ok());
#ifndef KRB5_VERSION_LE_1_10
