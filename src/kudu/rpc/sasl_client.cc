...
Status SaslClient::Negotiate() {
  ...
  while (!nego_ok_ || nego_response_expected_) {
    ...
    RETURN_NOT_OK(ParseNegotiateResponse(header, param_buf, &response));
...
Status SaslClient::SendNegotiateMessage(const NegotiatePB& msg) {
...
Status SaslClient::SendNegotiateMessage() {
  ...
  RETURN_NOT_OK(SendNegotiateMessage(msg));
...
Status SaslClient::SendResponseMessage(const char* resp_msg, unsigned resp_msg_len) {
  ...
  RETURN_NOT_OK(SendNegotiateMessage(reply));
  nego_response_expected_ = true;
  return Status::OK();
}
