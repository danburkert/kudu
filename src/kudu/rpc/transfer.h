...
class OutboundTransfer : public boost::intrusive::list_base_hook<> {
 public:
  ...
  // is triggered.
  //
  // The required features are checked against the set of features that the
  // server supports. This happens at the transfer level instead of while the
  // call is being queued, because the call may be queued before the negotiation
  // with the server has taken place. The call_id allows the call to be canceled
  // if required features are not supported.
  //
  // NOTE: 'payload' is currently restricted to a maximum of kMaxPayloadSlices
  // slices.
  OutboundTransfer(uint64_t call_id,
