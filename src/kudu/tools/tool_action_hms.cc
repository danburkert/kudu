...
Status Init(const RunnerContext& context,
  ...
  if (FLAGS_hive_metastore_uris.empty()) {
    ...
    RETURN_NOT_OK(GetServerFlags(master_addrs[0], master::Master::kDefaultPort, true, {}, &flags));

    auto hms_uris = std::find_if(flags.begin(), flags.end(),
        [] (const GetFlagsResponsePB_Flag& flag) {
          return flag.name() == "hive_metastore_uris";
        });
    if (hms_uris == flags.end() || hms_uris->value().empty()) {
    ...
    // If the URIs flag is present then the SASL flag must also be present,
    // since they were introduced at the same time.
    CHECK(hms_sasl_enabled != flags.end());
