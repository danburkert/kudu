...
Status Log::Init() {
  lock_guard<percpu_rwlock> write_lock(state_lock_);
...
Status Log::AsyncAllocateSegment() {
  lock_guard<boost::shared_mutex> lock_guard(allocation_lock_);
...
Status Log::DoAppend(LogEntryBatch* entry_batch, bool caller_owns_operation) {
  ...
  if (entry_batch->type_ == REPLICATE) {
    ...
    lock_guard<rw_spinlock> write_lock(last_entry_op_id_lock_);
...
Status Log::GC(int64_t min_op_idx, int32_t* num_gced) {
  ...
  VLOG_TIMING(1, "Log GC") {
    ...
    {
      lock_guard<percpu_rwlock> l(state_lock_);
      CHECK_EQ(kLogWriting, log_state_);
...
void Log::SetSchemaForNextLogSegment(const Schema& schema,
                                     uint32_t version) {
  lock_guard<rw_spinlock> l(schema_lock_);
  schema_ = schema;
