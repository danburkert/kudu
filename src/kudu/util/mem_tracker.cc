...
void MemTracker::Consume(int64_t bytes) {
  ...
  for (auto& elem : all_trackers_) {
    elem->consumption_.IncrementBy(bytes);
    if (!elem->consumption_func_.empty()) {
      DCHECK_GE(elem->consumption_.current_value(), 0);
    }
  }
}
...
bool MemTracker::AnyLimitExceeded() {
  for (auto& elem : limit_trackers_) {
    if (elem->LimitExceeded()) {
      return true;
    }
  }
  return false;
...
int64_t MemTracker::SpareCapacity() const {
  ...
  for (const auto& elem : limit_trackers_) {
    int64_t mem_left = elem->limit() - elem->consumption();
