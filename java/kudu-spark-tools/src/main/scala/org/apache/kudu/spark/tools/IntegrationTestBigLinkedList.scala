...
object IntegrationTestBigLinkedList {
  ...
  def usage: String =
    s"""
       | Usage: ${getClass.getSimpleName} COMMAND [COMMAND options]");
       |    where COMMAND is one of:
       |
       |    generate                   A Spark job that generates linked list data.
       |
       |    verify                     A Spark job that verifies generated
       |                               linked list data. Look at the counts
       |                               after running REFERENCED and
       |                               UNREFERENCED are ok any UNDEFINED counts
       |                               are bad. Do not run at the same time as
...
object Generator {
  ...
  def usage: String =
    s"""
       | Usage: generate [--tasks] [--lists] [--nodes] [--hash-partitions] [--range-partitions]
       |                 [--replicas] [--master-addrs] [--table-name]
       |    where
       ...
       |      hashPartitions: number of hash partitions to create for new linked list table, if it doesn't exist, default: 1
       |      rangePartitions: number of range partitions to create for the new linked list table, if it doesn't exist, default: 1
       |      replicas: number of replicas to create for new tables, if they don't exist, default: 1
  ...
  def createBatch(batchSize: Int, rand: Xoroshiro128PlusRandom): Array[(Long, Long)] = {
...
object Verifier {
  ...
  def usage: String =
    s"""
       | Usage: verify [--nodes] [--master-addrs] [--table-name]
       |    where
  ...
  def run(args: Args, sc: SparkContext): Counts = {
    ...
    sql.sql(
      s"""
         ...
         | FROM nodes t1
     """.stripMargin).createTempView("ref_counts")
    ...
    sql.sql(
      s"""
         ...
         |             ELSE "OVERREFERENCED" END as state,
         |        nodes
         | FROM ref_counts
       """.stripMargin).createOrReplaceTempView("ref_counts")
    ...
    sql.sql(
      s"""
       ...
       """.stripMargin).createTempView("undefined")
    ...
    rows.foldLeft(Counts(0, 0, 0, 0))((counts, row) => {
      ...
      state match {
        ...
        case "OVERREFERENCED" => counts.copy(overreferenced = count)
      }
    })
  }
  ...
  def main(arguments: Array[String]): Unit = {
    ...
    var success = true
    if (args.nodes.exists(_ != counts.referenced)) {
