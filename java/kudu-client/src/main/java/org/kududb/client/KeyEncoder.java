...
class KeyEncoder {
   ...
   *
   * TODO: this method uses the column IDs of the partition schema as column indexes in the
   * schema and row. This is currently fine because the column ID and column index always
   * match for primary key columns, but it should be refactored.
   *
  ...
  private void encodeColumn(PartialRow row, int columnIdx, boolean isLast) {
    ...
    if (!row.isSet(columnIdx)) {
      throw new IllegalStateException(String.format("Primary key column %s is not set", column));
    }
